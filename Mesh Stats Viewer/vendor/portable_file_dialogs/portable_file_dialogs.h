            ...
            opt options /* = opt::none */)
{
                         ...
                         options](int *exit_code) -> std::string
    {
        ...
        if (!m_wdefault_path.empty())
        {
            ...
            {
                const auto last_backslash_pos = m_wdefault_path.rfind('\\');
                const auto filename = m_wdefault_path.substr(last_backslash_pos + 1);
                m_wdefault_path = m_wdefault_path.substr(0, last_backslash_pos);

                path_attr = GetFileAttributesW(m_wdefault_path.c_str());
                if (path_attr != INVALID_FILE_ATTRIBUTES && (path_attr & FILE_ATTRIBUTE_DIRECTORY))
                    ofn.lpstrInitialDir = m_wdefault_path.c_str();

                // Second argument is size of buffer, not length of string
                StringCchCopyW(ofn.lpstrFile, MAX_PATH * 256 + 1, filename.c_str());
            }
            else
            {
